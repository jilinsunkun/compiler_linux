...
scanf("\n%c,%d,%d,%d",&tasks[i].task_id,&tasks[i].ci,&tasks[i].ti,&tasks[i].di);       printf("ID=%c,Ci=%d,Ti=%d,Di=%d\n",tasks[i].task_id,tasks[i].ci,tasks[i].ti,tasks[i].di);tasks[i].ci_left=tasks[i].ci;tasks[i].ti_left=tasks[i].ti;
            ...
            tasks[i].arg=i;
            tasks[i].call_num=1;      
            sum=sum+(float)tasks[i].ci/(float)tasks[i].ti;}}else{for(i=0; i<task_num; i++){printf("请输入任务ID，Ci，Ti：");scanf("\n%c,%d,%d",&tasks[i].task_id,&tasks[i].ci,&tasks[i].ti);
...
pthread_create(&tasks[i].th,NULL,(void*)proc,&tasks[i].arg);
    for(i=0; i<demo_time; i++){int j;
        if((curr_proc=select_proc(alg))!=-1){
           // 按调度算法选线程
            pthread_mutex_unlock(&proc_wait[curr_proc]);            pthread_mutex_lock(&main_wait);  //主线程等待}
        else{//无可运行任务，选择闲逛线程pthread_mutex_unlock(&idle_wait);
            pthread_mutex_lock(&main_wait);}
        for(j=0; j<task_num; j++){if(alg==1)tasks[j].di_left--;//Ti--，为0时开始下一周期if(--tasks[j].ti_left==0){
